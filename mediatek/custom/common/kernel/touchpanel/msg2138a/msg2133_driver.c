...
{	
	printk("tpd_touchinfo gesture--%x,%x,%x,%x,%x,%x \n",val[1],val[2],val[3],val[4],val[5],val[6]);
...
#endif
	//msg_Gesture_Fucntion_Proc_File();
#ifdef PROC_FIRMWARE_UPDATE
	_msg_create_file_for_fwUpdate_proc();
	//msg_update_Fucntion_Proc_File();
#endif
#ifdef ITO_TEST
...
#ifdef WT_CTP_GESTURE_SUPPORT
	int closeGesturnRetval = 3;
while(closeGesturnRetval--){
 	if(1==msg_CloseGestureFunction())
		break;
}
#endif
