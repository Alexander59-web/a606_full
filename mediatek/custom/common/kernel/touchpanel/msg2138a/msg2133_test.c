...
extern void HalTscrCReadI2CSeq(u8 addr, u8* read_data, u16 size);
...
u32 ito_test_choose_TpType(void)
{
    ...
    for(i=0;i<10;i++)
    {
        ...
        if(TP_OF_OFILM==tpType)//modify:注意该项目tp数目
        {
            break;
        }
        else if(i<5)
...
int msg_OpenGestureFunction( int g_Mode )
 {
	 ...
	 */
	 ...
	 {
		 TP_DEBUG("***msg_OpenGestureFunction :the command is wrong!***\n");
	 ...
	 TP_DEBUG("***msg_OpenGestureFunction The actual Gesture Function mode is = %x ***\n", ((dbbus_rx_data[1]<<8)+dbbus_rx_data[0]));
 
	 if(((dbbus_rx_data[1]<<8)+dbbus_rx_data[0]) != g_Mode)
	 {
		 tpd_gesture_flag = 0;
		 TP_DEBUG("***msg_OpenGestureFunction fail***\n");
		 return 0;
	 }
	 else
	 {
		 tpd_gesture_flag = 1;
		 TP_DEBUG("***msg_OpenGestureFunction success***\n");
		 return 1;
	 }
	 /**********check command*********/
#ifdef __MSG_DMA_MODE__
 ...
 void tp_Gesture_Fucntion_Proc_File(void)
  {  
		   ...
		   else 
		   {
			   ...
			   TPD_DEBUG("ctp_gesture_var_proc create success\n");
		   }
		   ...
		   else 
		   {
			   ...
			   TPD_DEBUG("ctp_gesture_type_proc create success\n");
		   }
		   ...
		   else 
		   {
			   ...
			   TPD_DEBUG("ctp_gesture_onoff_proc create  success\n");
		   }  
		   ...
		   else 
		   {
			   ...
			   TPD_DEBUG("ctp_glove_onoff_proc create success\n");
		   }
	  ...
	  ///////////////uupdate   
		  ...
		  else 
		  {
			  ...
			  TPD_DEBUG("create_proc_entry version success\n");
		  }
