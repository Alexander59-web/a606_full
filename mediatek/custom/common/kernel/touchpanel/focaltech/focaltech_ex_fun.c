...
extern s32 fts_i2c_smbus_read_i2c_block_data(const struct i2c_client *client,\
                                          u8 command, u8 length, u8 *values);

extern s32 fts_i2c_smbus_write_i2c_block_data(const struct i2c_client *client,\
...
static DEFINE_MUTEX(i2c_access);//yanghaichao  add  20140301
	...
	if(writelen!=0)
	{
		...
		else
		{
			...
			if((ret=i2c_master_send(client, (unsigned char *)I2CDMABuf_pa, writelen))!=writelen)
				dev_err(&client->dev, "###%s ret=%d i2c write len=%x,buffaddr=%x\n", __func__,ret, writelen, I2CDMABuf_pa);
					...
					const char *buf, size_t count)
{
	...
	if (2 == num_read_chars) {
		/*read register*/
		regaddr = wmreg;
		if(fts_i2c_smbus_read_i2c_block_data(client, regaddr, 1, &regvalue) < 0)
		...
		regaddr = wmreg >> 8;
		regvalue = wmreg;
		if (fts_i2c_smbus_write_i2c_block_data(client, regaddr, 1, &regvalue) < 0)
...
#define FOCAL_TPG
#ifdef FOCAL_TPG
#define PROC_GET_FACEDETECT_STATE			0x13
#define PROC_GET_PSENSOR_STATE				0x14
#define PROC_GET_GSENSOR_SMALLMOVE_THR		0x15
#define PROC_GET_GSENSOR_BIGMOVE_THR		0x16
#define PROC_SET_GSENSOR_CHANGE_STATE		0x17

#define GSENSOR_SMALLMOVE_THR	120
#define GSENSOR_BIGMOVE_THR		240
#define GSENSOR_CHANGE_STATE_REG	0x8E

#ifdef TPD_PROXIMITY
extern u8 fts_tpd_proximity_detect;
extern u8 fts_tpd_proximity_flag;
#endif
#endif
...
static unsigned char proc_register_addr = 0x00;
	...
	const char __user *buff, unsigned long len, void *data)
{
	...
	case PROC_READ_REGISTER:
		...
		#else
		proc_register_addr = writebuf[1];
		#endif
		break;
	case PROC_WRITE_REGISTER:
		writelen = 2;
		//ret = fts_i2c_Write(client, writebuf + 1, writelen);
		ret = fts_i2c_smbus_write_i2c_block_data(client, writebuf[1], 1, &writebuf[2]);
		if (ret < 0) {
			dev_err(&client->dev, "%s:write iic error\n", __func__);
			return ret;
		}
	...
	case PROC_WRITE_DATA:
		...
		if(ret<0)
		{
			dev_err(&client->dev,"%s:wrte iic error\n",__func__);
			return ret;		
		}
		break;
		#ifdef FOCAL_TPG
	case PROC_SET_GSENSOR_CHANGE_STATE:
		printk("focal----------------proc set gsensor change state. state = 0x%02x\n", writebuf[1]);
		ret = fts_i2c_smbus_write_i2c_block_data(client, GSENSOR_CHANGE_STATE_REG,\
			1, &writebuf[1]);
	//	GSensorStateCmd[0] = GSENSOR_CHANGE_STATE_REG;
	//	GSensorStateCmd[1] = writebuf[1];
		//ret = fts_i2c_Write(client, GSensorStateCmd, 2);
		if (ret < 0) {
			dev_err(&client->dev, "%s:write iic error\n", __func__);
			return ret;
		}
	...
	off_t off, int count, int *eof, void *data )
{
	...
	case PROC_READ_REGISTER:
		...
		if (ret < 0) {
			...
			DBG("%s:value=0x%02x\n", __func__, buf[0]);
		#else
		regaddr = proc_register_addr;
		ret = fts_i2c_smbus_read_i2c_block_data(client, regaddr, 1, buf);
		//ret = fts_i2c_Read(client, &regaddr, 1, buf, readlen);
		if (ret < 0) {
			dev_err(&client->dev, "%s:read iic error\n", __func__);
			return ret;
		} else
	...
	#ifdef FOCAL_TPG
	#ifdef TPD_PROXIMITY
	case PROC_GET_FACEDETECT_STATE:
		buf[0] = fts_tpd_proximity_detect;
		num_read_chars = 1;
		break;
	case PROC_GET_PSENSOR_STATE:
		buf[0] = fts_tpd_proximity_flag;
		num_read_chars = 1;
		break;
	#endif
	case PROC_GET_GSENSOR_SMALLMOVE_THR:
		buf[0] = GSENSOR_SMALLMOVE_THR >> 8;
		buf[1] = GSENSOR_SMALLMOVE_THR & 0x00FF;
		num_read_chars = 2;
		break;
	case PROC_GET_GSENSOR_BIGMOVE_THR:
		buf[0] = GSENSOR_BIGMOVE_THR >> 8;
		buf[1] = GSENSOR_BIGMOVE_THR & 0x00FF;
		num_read_chars = 2;
		break;
#endif
	default:
