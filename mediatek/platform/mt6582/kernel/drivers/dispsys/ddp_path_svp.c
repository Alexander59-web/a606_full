...
static int disp_backup_module_reg (DISP_MODULE_ENUM module);
static int disp_restore_module_reg (DISP_MODULE_ENUM module);
...
extern unsigned int secure_port_flag(void);
...
extern unsigned int need_release_rdma_dapc;
extern unsigned int is_device_apc_rdma;
...
int disp_check_engine_status(int mutexID)
{
    ...
    if(engine&DDP_OVL_UPDATE_BIT) //OVL
    {
           ...
           ((DISP_REG_GET(DISP_REG_OVL_FLOW_CTRL_DBG)&0x3ff) != 0x2))
        {
            ...
            ddp_dump_info(DISP_MODULE_MUTEX);
            ddp_dump_info(DISP_MODULE_OVL);
            MMProfileLog(MTKFB_MMP_Events.OvlReset, MMProfileFlagPulse);
        	disp_path_ovl_reset();
        }
    }
...
int disp_path_config_rdma (RDMA_CONFIG_STRUCT* pRdmaConfig) {
...
#if defined(CONFIG_TRUSTONIC_TEE_SUPPORT) && defined(CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT)
    if (1 == secure_port_flag() || 1 == is_decouple_secure_port_wdma) {
    MMProfileLog(MTKFB_MMP_Events.ConfigRDMASecure, MMProfileFlagStart);
        switch_rdma_apc(1);
        config_rdma_secure((void *)pRdmaConfig, sizeof(RDMA_CONFIG_STRUCT));
        switch_rdma_secure_port(1);
    MMProfileLog(MTKFB_MMP_Events.ConfigRDMASecure, MMProfileFlagEnd);
    } else {
        if (is_device_apc_rdma == 0) {
#endif
	RDMAConfig(pRdmaConfig->idx, pRdmaConfig->mode, pRdmaConfig->inputFormat, pRdmaConfig->address,
        ...
        switch_rdma_secure_port(0);
	        ...
	        MMProfileLog(MTKFB_MMP_Events.ConfigRDMASecure, MMProfileFlagPulse);
	        config_rdma_secure((void *)pRdmaConfig, sizeof(RDMA_CONFIG_STRUCT));
            need_release_rdma_dapc = 1;
	    }
    }
#endif
	return 0;
...
/*
 * Each module has it's own address offset, so you can just only do this for
 * one or more each time safely
 * Module Offset pointer:
 * DISP_MODULE_CONFIG --> [0x000 ~ 0x100)
 * DISP_MODULE_MUTEX  --> [0x100 ~ 0x200)
 * DISP_MODULE_OVL	  --> [0x200 ~ 0x300)
 * DISP_MODULE_WDMA0  --> [0x300 ~ 0x400)
 * DISP_MODULE_RDMA   --> [0x400 ~ 0x500)
 * DISP_MODULE_RDMA1  --> [0x500 ~ 0x600)
 * others append here...
 */
static int disp_backup_module_reg (DISP_MODULE_ENUM module)
{
	unsigned int index = 0;
	unsigned int reg_offset_start = 0;
	switch (module)
	{
	case DISP_MODULE_CONFIG:
		reg_offset_start = reg_offset = 0x0;
		reg_backup(DISP_REG_CONFIG_DISP_OVL_MOUT_EN );
		reg_backup(DISP_REG_CONFIG_DISP_OUT_SEL     );
	break;

	case DISP_MODULE_MUTEX:
		reg_offset_start = reg_offset = 0x100;
		disp_mutex_backup();
		break;

	case DISP_MODULE_OVL:
		reg_offset_start = reg_offset = 0x200;
	...
	case DISP_MODULE_WDMA0:
		reg_offset_start = reg_offset = 0x300;
	...
	case DISP_MODULE_RDMA1:
		index = 1;
	case DISP_MODULE_RDMA:
		reg_offset_start = reg_offset = 0x400 + index*0x100;
	...
	DISP_MSG("disp_backup_module_reg() , *pRegBackup=0x%x, module=%d, start=0x%x, end=0x%x\n",
			*pRegBackup, module, reg_offset_start, reg_offset);
	return 0;
}
...
static int disp_restore_module_reg (DISP_MODULE_ENUM module)
{
	...
	DISP_MSG("disp_restore_module_reg() , *pRegBackup=0x%x, module=%d, start=0x%x, end=0x%x\n",
			*pRegBackup, module, reg_offset_start, reg_offset);
	return 0;
}
...
static int disp_reg_backup(void)
{
	disp_backup_module_reg(DISP_MODULE_CONFIG);
	disp_backup_module_reg(DISP_MODULE_MUTEX);
	disp_backup_module_reg(DISP_MODULE_OVL);
	disp_backup_module_reg(DISP_MODULE_RDMA);
	disp_backup_module_reg(DISP_MODULE_RDMA1);
	disp_backup_module_reg(DISP_MODULE_WDMA0);
...
static int disp_reg_restore(void)
{
    disp_restore_module_reg(DISP_MODULE_CONFIG);
    disp_restore_module_reg(DISP_MODULE_MUTEX);
    disp_restore_module_reg(DISP_MODULE_RDMA);
    disp_restore_module_reg(DISP_MODULE_RDMA1);
    disp_restore_module_reg(DISP_MODULE_OVL);
    disp_restore_module_reg(DISP_MODULE_WDMA0);
