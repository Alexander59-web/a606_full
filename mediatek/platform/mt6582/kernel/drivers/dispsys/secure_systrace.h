...
enum {
    ...
    SEC_SYSTRACE_MODULE_MEM = 5,
    SEC_SYSTRACE_MODULE_DAPC = 6,
};
...
#define THREAD_ID_BASE  (50000)
#define CPU_ID_BASE	    (8)
...
static int systrace_seq_show(struct seq_file *m, void *v)
{
    ...
    if (*(loff_t *)v == 0) {
        ...
        seq_printf(m, "%16s-%-6u[%03u]%7llu.%06u: sched_switch: prev_comm=dummyProc"
                    ...
                    "dummyProc", 0, module_id+CPU_ID_BASE, second, microsecond, log_name, module_id+THREAD_ID_BASE);
    }
    ...
    if (log->type == 0) {
        seq_printf(m, "%16s-%-6u[%03u]%7llu.%06u: tracing_mark_write: B|%u|%s\n",
                    log_name, module_id+THREAD_ID_BASE, module_id+CPU_ID_BASE, second, microsecond, module_id+THREAD_ID_BASE, log->name);
    } else {
        seq_printf(m, "%16s-%-6u[%03u]%7llu.%06u: tracing_mark_write: E\n",
                    log_name, module_id+THREAD_ID_BASE, module_id+CPU_ID_BASE, second, microsecond);
    }
...
static int _systrace_init(const unsigned int id, const char *module,
        const char *name, const size_t buf_size, struct secure_callback_funcs *callback,
        const int alloc_mode)
{
    char proc_name[64];
...
static void _systrace_deinit(const char *module, const int alloc_mode)
{
    char proc_name[64];
...
static int systrace_init(const unsigned int id, const char *module,
        const char *name, const size_t buf_size, struct secure_callback_funcs *callback)
{
    return _systrace_init(id, module, name, buf_size, callback, 0);
}

static void systrace_deinit(const char *module)
{
    _systrace_deinit(module, 0);
}

static int systrace_init1(const unsigned int id, const char *module,
        const char *name, const size_t buf_size, struct secure_callback_funcs *callback)
{
    return _systrace_init(id, module, name, buf_size, callback, 1);
}

static void systrace_deinit1(const char *module)
{
    _systrace_deinit(module, 1);
}
