...
struct ipanic_header* ipanic_header_from_sd(void) {
	...
	if(IS_ERR_OR_NULL((void*)header)) {
		IPANIC_D("read header failed[%d]\n", header);
		header = NULL;
	} else if(header->magic != AEE_IPANIC_MAGIC) {
		IPANIC_D("no ipanic data[%x]\n", header->magic);
		header = NULL;
		ipanic_erase();
	} else {
		...
		IPANIC_D("ipanic data available^v^%s^v^\n", str);
	}
	return header;
}
...
struct ipanic_header* ipanic_header(void) {
	...
	if (ipanic_msdc_info(iheader)) {
		IPANIC_E("ipanic initialize msdc fail.");
		aee_nested_printf("$");
		return NULL;
	}
