...
char *ion_get_backtrace_info(struct ion_record_basic_info *tracking_info,char *backtrace_string,unsigned int backtrace_string_len, unsigned int backtrace_index,unsigned int show_backtrace_type)
{
	...
	if(backtrace_info == USER_BACKTRACE)
		...
		if(tmpMapping != NULL)
		{
			...
			{
				if((backtrace_mapping[backtrace_index].name != NULL) && (backtrace_mapping[backtrace_index].size > 0))
				{
				...
				{
					snprintf(backtrace_string,backtrace_string_len,"USER[%d] addr: 0x%x \n",backtrace_index,backtrace[backtrace_index],backtrace_mapping[backtrace_index].address);
				}
		...
		if(tmpMapping != NULL)
			...
			if(backtrace_symbol != NULL)	
			{
				...
				return backtrace_string;
                        ...
                        {
				snprintf(backtrace_string,backtrace_string_len,"KERNEL[%d] addr: 0x%x symbol: no backtrace symbol\n",backtrace_index,backtrace[backtrace_index]);
                                ION_DEBUG_LOG(ION_DEBUG_ERROR, "[ion_get_backtrace_info]ERROR backtrace_mapping is NULL\n");
                        }
		}
		else
...
char *get_string(char *string_name,unsigned int len,StringTable *table,struct mutex *string_mutex)
{
	...
	if(entry == NULL)
	{
		ION_DEBUG_LOG(ION_DEBUG_ERROR, "ERROR!! get_string can't get enough memory\n");
		return NULL;
	}
	...
	if(entry->name == NULL)
	{
	        ION_DEBUG_LOG(ION_DEBUG_ERROR, "ERROR!! get_string can't get enough memory for name len = %d\n",len);
                return NULL;
	}
		...
		switch(type)
		{
			...
			case HASH_NODE_BUFFER:
			{
				...
				else
				{
					...
					if(entry == NULL)
					{
						ION_DEBUG_LOG(ION_DEBUG_ERROR, "ERROR!! get_record can't get enough memory for buffer object\n");
 						return NULL;
			...
			case HASH_NODE_USER_BACKTRACE:
			{
				...
				else
				{
					...
					if(entry == NULL)
					{
						ION_DEBUG_LOG(ION_DEBUG_ERROR, "ERROR!! get_record can't get enough memory for user backtrace\n");
						return NULL;
					}
					entry->reference = 1;
					entry->prev = NULL;
			...
			case HASH_NODE_KERNEL_BACKTRACE:
			{
				...
				mutex_unlock(&gKernelBtTable_mutex);
					...
					if(entry == NULL)
					{
						ION_DEBUG_LOG(ION_DEBUG_ERROR,"ERROR!! get_record can't get enough memory for KERNEL BACKTRACE backtrace num %d\n",param->backtrace_num);
						return NULL;
					}
                                        entry->reference = 1;
                                        entry->prev = NULL;
			...
			case HASH_NODE_USER_MAPPING:
			{
					...
					if(entry == NULL)
					{
						ION_DEBUG_LOG(ION_DEBUG_ERROR,"ERROR!! get_record can't get enough memory for USER MAPPING backtrace num %d\n",param->backtrace_num);
						return NULL;
					}
                                        entry->reference = 1;
                                        entry->prev = NULL;
			...
			case HASH_NODE_KERNEL_SYMBOL:
			{
				hash = get_hash(param->kernel_symbol,param->backtrace_num);
					...
					if(entry == NULL)
					{
						ION_DEBUG_LOG(ION_DEBUG_ERROR,"ERROR!! get record can't get enough memory for KERNEL symbol %d\n",param->backtrace_num);
						return NULL;	
					}
                                        entry->reference = 1;
                                        entry->prev = NULL;
