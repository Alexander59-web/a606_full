...
static void *allocate_record(unsigned int type)
{
	switch(type)
	{
		case NODE_PAGE_RECORD:
		{
			...
			if(page_cachep != NULL)
			{
				if(page_record_total < page_recorder_limit)
				{
					void *tmp = NULL;
				...
				{
					if( 0 == page_recorder_debug_max)
					{
						page_recorder_debug_max = 2;
					}
				}				
			}
			return NULL;
...
PageHashEntry* record_page_info(PageObjectEntry* bt_entry,PageObjectEntry* map_entry, void* page, unsigned int order, unsigned int flag)
{
	...
	if(page_record_count >= 1000)
        {
                page_recorder_memory_usage = page_record_total*sizeof(PageHashEntry) 
					     + bt_record_total*(sizeof(PageObjectEntry)+(20 * sizeof(unsigned int))) 
					     + gKernelPageSymbolTable.count * sizeof(PageObjectEntry)+(10 * sizeof(unsigned int));
...
int record_page_record(void *page, unsigned int order)
{
	...
	if((!page_recorder_debug) || (page == NULL) || (order >= MAX_ORDER))
	{
		return 0;
	}
	if(page_record_lock_init == 0)
...
static int page_recorder_debug_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "page_recorder_usage: [%d]\n", page_recorder_memory_usage);
	seq_printf(s, "page_recorder_debug_max: [%d]\n", page_recorder_debug_max);
